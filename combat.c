...
void effect_status(Pokemon* poke, int* nbtour, SDL_Renderer* rend, SDL_Window* wind){//renwind
...
Comp choix_atk(Pokemon* bu, SDL_Renderer* rend, SDL_Window* wind){//renwind
    ...
    do{
        ...
        strcpy(name, get_name_from_mouse(2, &x, &y, *bu, NULL, NULL));
...
float degats(Pokemon* attaquant, Pokemon* defenseur, Comp atq, SDL_Renderer* rend, SDL_Window* wind){//renwind
...
void effet_spe(Pokemon** attaquant, Pokemon* defenseur, Comp atk, SDL_Renderer* rend, SDL_Window* wind) {//renwind
    int alea = rand() % 100;
    ...
    switch (atk.comp) {
        ...
        case Giga_sangsue:
            if(((*attaquant)->pv_courant+(degats((*attaquant), defenseur, atk, rend, wind)) / 2)>(*attaquant)->pv_max){
                (*attaquant)->pv_courant=(*attaquant)->pv_max;
            }
            else{
                (*attaquant)->pv_courant += (degats((*attaquant), defenseur, atk, rend, wind)) / 2;
            }
            printf("\n");
            aff_char((*attaquant)->nom_poke);
...
Comp maj_damage(Pokemon* offense, Pokemon** adversaires, Pokemon** cible, SDL_Renderer* rend, SDL_Window* wind){
    ...
    *cible=adversaires[0];
    for(int j=0; j<3; j++){
        if(degats(offense, adversaires[j], offense->atkbase, rend, wind)>max){
            result=offense->atkbase;
            *cible=adversaires[j];
            max=degats(offense, adversaires[j], offense->atkbase, rend, wind);
            adr=&(offense->atkbase);
        }
        if((degats(offense, adversaires[j], offense->spe1, rend, wind)>max) && offense->spe1.cooldown==0){
            result=offense->spe1;
            *cible=adversaires[j];
            max=degats(offense, adversaires[j], offense->spe1, rend, wind);
            adr=&(offense->spe1);
        }
        if((degats(offense, adversaires[j], offense->spe2, rend, wind)>max) && offense->spe2.cooldown==0){
            result=offense->spe2;
            *cible=adversaires[j];
            max=degats(offense, adversaires[j], offense->spe2, rend, wind);
            adr=&(offense->spe2);
        }
        if((degats(offense, adversaires[j], offense->spe3, rend, wind)>max) && offense->spe3.cooldown==0){
            result=offense->spe3;
            *cible=adversaires[j];
            max=degats(offense, adversaires[j], offense->spe3, rend, wind);
            adr=&(offense->spe3);
        }
    }
...
void fin_e_a_b(Pokemon* offense, Pokemon* cible, Comp choix, SDL_Renderer* rend, SDL_Window* wind){//renwind
    int x=0;
    ...
    x=degats(offense, cible, choix, rend, wind);
    if(cible->protec==1){
        printf("\n\n");
        aff_char(cible->nom_poke);
        x=0;
    ...
    effet_spe(&offense, cible, choix, rend, wind);
...
void effect_atk_bot(Pokemon* offense, Pokemon** team, Pokemon** adversaires, SDL_Renderer* rend, SDL_Window* wind){//renwind
    printf("\n\n%s", (*offense).nom_poke);
    Pokemon* cible;
    cible=NULL;
    Comp choix;
    for(int j=0; j<3; j++){
        if(((degats(offense, adversaires[j], offense->atkbase, rend, wind))>adversaires[j]->pv_courant) && (adversaires[j]->pv_courant>0)){
            cible=adversaires[j];
    ...
    if(cible==NULL){
        for(int j=0; j<3; j++){
            if(((degats(offense, adversaires[j], offense->spe1, rend, wind)>adversaires[j]->pv_courant && offense->spe1.cooldown==0)) && (adversaires[j]->pv_courant>0)){
                cible=adversaires[j];
                choix=offense->spe1;
    ...
    if(cible==NULL){
        for(int j=0; j<3; j++){
            if(((degats(offense, adversaires[j], offense->spe2, rend, wind)>adversaires[j]->pv_courant && offense->spe2.cooldown==0)) && (adversaires[j]->pv_courant>0)){
                cible=adversaires[j];
                choix=offense->spe2;
    ...
    if(cible==NULL){
        for(int j=0; j<3; j++){
            if(((degats(offense, adversaires[j], offense->spe3, rend, wind)>adversaires[j]->pv_courant && offense->spe3.cooldown==0)) && (adversaires[j]->pv_courant>0)){
                cible=adversaires[j];
                choix=offense->spe3;
    ...
    if(choix.comp==Abri || choix.comp==Repos || choix.comp==Plenitude || choix.comp==Gonflette || choix.comp==Mur_de_fer){
        ...
        fin_e_a_b(offense, cible, choix, rend, wind);
    }
}
Pokemon* choix_target(Pokemon** team, Pokemon** opp, SDL_Renderer* rend, SDL_Window* wind){
    ...
    name=get_name_from_mouse(3, &x, &y, **team, team, opp);
    if(name[0]=='1'){
        return opp[0];
    }
    if(name[0]=='2'){
...
void fin_e_a(Pokemon* offense, Pokemon* cible, Comp atk, SDL_Renderer* rend, SDL_Window* wind){//renwind
    int x=0;
    int aleapara=rand()%100+1;
    int aleagel=rand()%100+1;
    aff_atk_effect_sdl(rend, offense, atk, cible);
    printf("\n");
    aff_char(offense->nom_poke);//utilise atk
    printf(" utilise ");
    aff_atk(atk.comp);           
    x=degats(offense, cible, atk, rend, wind);
    if(cible->protec==1){
        printf("\n\n");
        aff_char(cible->nom_poke);
        x=0;
    ...
    effet_spe(&offense, cible, atk, rend, wind);
    if(cible->protec==1){
        x=0;
        printf(" s'est protege");
        cible->protec=0;
...
void effect_atk(Pokemon* offense, Pokemon** team, Pokemon** adversaires, Comp atk, SDL_Renderer* rend, SDL_Window* wind){//renwind
    Pokemon* cible;
        ...
        if(atk.comp==Aboiement || atk.comp==Abattage){
            for(int o=0; o<3; o++){
                if (adversaires[o] != NULL && adversaires[o]->pv_courant > 0){
                    fin_e_a(offense, adversaires[o], atk, rend, wind);
                }
            }            
        }
        else if(atk.comp==Seisme || atk.comp==Surf){
            offense->pv_courant+=degats(offense, offense, atk, rend, wind);
            for(int o=0; o<3; o++){
                if (team[o] != NULL && team[o]->pv_courant > 0){
                    fin_e_a(offense, team[o], atk, rend, wind);
                }
            }            
            for(int o=0; o<3; o++){
                if (adversaires[o] != NULL && adversaires[o]->pv_courant > 0){
            ...
            fin_e_a(offense, offense, atk, rend, wind);
        }
    }
    else{
        cible=choix_target(team, adversaires, rend, wind);
        fin_e_a(offense, cible, atk, rend, wind);
    }
}
...
int fight(Pokemon** player, Pokemon** bot, int mode, SDL_Renderer* rend, SDL_Window* wind) { //renwind
    ...
    aff_fenetre(7, wind, rend, surface2, icon2, texture2_2);
    printf("\nOk");
        ...
        while ((player[0]->pv_courant > 0 || player[1]->pv_courant > 0 || player[2]->pv_courant > 0) && (bot[0]->pv_courant > 0 || bot[1]->pv_courant > 0 || bot[2]->pv_courant > 0)) {
            ...
            if (plus1000(*player[0], *player[1], *player[2], *bot[0], *bot[1], *bot[2]) == 1) {
                ...
                if (maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]) == player[0] || maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]) == player[1] || maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]) == player[2]) {
                    effect_atk(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), player, bot, choix_atk(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), rend, wind), rend, wind);
                    effect_status(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), &maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2])->sleep, rend, wind);
                    cooldown1poke(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]));
                } 
                else {
                    effect_atk(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), bot, player, choix_atk(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), rend, wind), rend, wind);
                    effect_status(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), &maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2])->sleep, rend, wind);
                    cooldown1poke(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]));
                }
        ...
        while ((player[0]->pv_courant > 0 || player[1]->pv_courant > 0 || player[2]->pv_courant > 0) && (bot[0]->pv_courant > 0 || bot[1]->pv_courant > 0 || bot[2]->pv_courant > 0)) {
            ...
            if (plus1000(*player[0], *player[1], *player[2], *bot[0], *bot[1], *bot[2]) == 1) {
                ...
                if (maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]) == player[0] || maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]) == player[1] || maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]) == player[2]) {
                    ...
                    effect_atk_bot(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), bot, player, rend, wind);
                    effect_status(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]), &maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2])->sleep, rend, wind);
                    cooldown1poke(maj6(player[0], player[1], player[2], bot[0], bot[1], bot[2]));
                }
